	...
	for (let i = 0; i < children.length; i++) {
		...
		if (child.type === 1 && child.tagType === 0) {
			...
			if ((doNotHoistNode ? 0 : getConstantType(child, context)) >= 2) {
				if (child.codegenNode.type === 14 && child.codegenNode.arguments.length > 0) child.codegenNode.arguments.push(`-1 /* ${PatchFlagNames[-1]} */`);
				toCache.push(child);
				continue;
			}
		}
	...
	return function postTransformElement() {
		...
		if (node.children.length > 0) {
			if (vnodeTag === KEEP_ALIVE) {
				shouldUseBlock = true;
				patchFlag |= 1024;
				if (node.children.length > 1) context.onError(createCompilerError(47, {
...
var transformTransition = (node, context) => {
	if (node.type === 1 && node.tagType === 1) {
		if (context.isBuiltInComponent(node.tag) === TRANSITION) return () => {
			if (!node.children.length) return;
			if (hasMultipleChildren(node)) context.onError(createDOMCompilerError(63, {
				start: node.children[0].loc.start,
				end: node.children[node.children.length - 1].loc.end,
				source: ""
			}));
...
var DOMNodeTransforms = [transformStyle, ...[transformTransition, validateHtmlNesting]];
